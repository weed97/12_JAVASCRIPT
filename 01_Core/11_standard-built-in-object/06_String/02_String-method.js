...
// 문자열을ㄴ 변경 불가능한 원시 값이기 때문에 String 래퍼 객체도 읽기 전용 객체로 제공된다.
const obj = new String('홍길동');
console.log(Object.getOwnPropertyDescriptors(obj));     // writable : false
console.log('-----------------------------');

// String 객체의 모든 메서드는 String 래퍼 객체를 직접 변경 할 수 없고,
// String 객체의 메서드는 언제나 새로운 문자열 생성하여 반환한다.
...
// 특정 문자열 존재 유뮤 확인에 사용
if(str.indexOf('a') !== -1) console.log('a 가 있다');
console.log('---------------------------');

// String.prototype.includes
console.log(str.includes('a'));      // 문자열에서 a 검색하여 첫번째 인덱스 반환
...
console.log(str.startsWith('va', 4));       // 'Java'가 va로 끝나는지
console.log('------------------------------');
...
console.log(str.substring(1));      // 두번째 인수 생략시 문자열 끝까지
console.log(str.substring(4,1));    // 인수 교환하여 기능
console.log(str.substring(1,4));    // 음수는 0으로 취급
console.log(str.substring(1,4));    // length보다 크면 length로 취급
console.log('--------------------------------');
...
// console.log(str.repeat(-1));      //  음수는 에러 RangeError : Invaild count value
console.log('----------------------------');

// String.prototype.replace
// 첫번쨰 인수로 전달 받은 문자열 도는 정규표현식을 검색하여 두번째 인수로 전달한 문자열로 치환한 문자열 반환
console.log(str.replace('Java', 'Type'));
console.log(str.replace('a', 'b'));     // 검색 된 문자열이 여럿 존재할 경우 첫번째로 검색된 문자열만 치환
console.log(str.replace(/j/i, 'Z'));    // 첫번쨰 인수로 정규표현식 전달
console.log('--------------------------------');

// String.prototype.split
// 첫번째 인수로 전달한 문자열 또는 정규식을 검색하여 문자열을 구분한 후 분리 된 각 문자열로 이루어진 배열 반환
const str3 = 'Hello, Every! Nice to see you again.';
