...
// 정규표현식
// 정규 표현식 리터럴 :  /패턴/플래그
let regexp = /j/i; // i : 플래그, 대소문자 구분없이 j: /.../ j가 있는걸 패턴을 통해서 확인 가능하다.

// RegExp생성자 함수 : new RegExp(패턴, 플래그)
//
regexp = new RegExp('j','i')
regexp = new RegExp(/j/,'i')
// regexp = new RegExp(/j/,i) // 잘못된 사용방법

// test 메서드 - target 문자열에 대해 정규 표현식 regexp의 패턴을 검색하여 매칭 결과를 Boolean으로 변환
