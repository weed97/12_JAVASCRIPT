...
// length 프로퍼티에 임의의 숫자 값을 명시적으로할당 할 수 있다.
arr.length = 3;
console.log(arr); // 현제 length 프로퍼티보다 작은 값을 할당하면 배열의 길이가 줄어든다.

// 현제 length 프로퍼티보다 큰 숫자를 할당하면
// length 프로퍼티 값은 변경이되지만, 배열의 길이가 늘어나 지진 않는다.

arr.length = 10;
console.log(arr);
console.log(arr.length);
// 매서드에 주어진 객체에 모든 자체의 속성에 대한 설명을 반환
